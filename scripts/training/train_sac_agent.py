"""
Training script for SAC Agent in Bipedal Locomotion

This script trains a single SAC agent for bipedal locomotion tasks
...
class SACTrainer:
    """
    Trainer for SAC agent in bipedal locomotion.
    """
    ...
    def __init__(
        ...
        logger.info(f"Initialized SACTrainer for {self.env_name}")
    ...
    def _train_episode(self, episode: int) -> Tuple[float, int, Dict[str, float]]:
        ...
        for step in range(self.max_steps_per_episode):
            ...
            # Update agent (SAC updates every step)
    ...
    def _log_episode(self, episode: int, reward: float, length: int, metrics: Dict[str, float]):
        ...
        # Buffer size
        self.writer.add_scalar('Training/BufferSize', len(self.agent.replay_buffer), episode)
    ...
    def _plot_training_curves(self):
        ...
        ax1.set_title('SAC Training Rewards')
        ax1.grid(True)
        ...
        ax2.set_title('SAC Training Episode Lengths')
        ax2.grid(True)
        
        # Training metrics
        if self.training_metrics:
            actor_losses = [m.get('actor_loss', 0) for m in self.training_metrics]
            critic_losses = [m.get('critic1_loss', 0) for m in self.training_metrics]
            
            ax3.plot(actor_losses, label='Actor Loss', alpha=0.7)
            ax3.plot(critic_losses, label='Critic Loss', alpha=0.7)
            ax3.set_xlabel('Episode')
            ax3.set_ylabel('Loss')
            ax3.set_title('SAC Training Losses')
            ax3.legend()
            ax3.grid(True)
        
        # Alpha (temperature) curve
        if self.training_metrics:
            alphas = [m.get('current_alpha', 0) for m in self.training_metrics]
            ax4.plot(alphas, label='Alpha (Temperature)', color='orange')
            ax4.set_xlabel('Episode')
            ax4.set_ylabel('Alpha')
            ax4.set_title('SAC Temperature Parameter')
            ax4.legend()
            ax4.grid(True)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Train SAC agent for bipedal locomotion')
    ...
    parser.add_argument('--auto-entropy', action='store_true', default=True, help='Use automatic entropy tuning')
    ...
    config['agent_config']['buffer_size'] = args.buffer_size
    config['agent_config']['hidden_dim'] = args.hidden_dim
    config['agent_config']['automatic_entropy_tuning'] = args.auto_entropy
    ...
    trainer = SACTrainer(config, device)
    trainer.train()
